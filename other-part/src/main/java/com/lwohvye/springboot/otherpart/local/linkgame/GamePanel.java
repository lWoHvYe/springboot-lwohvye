...
public class GamePanel extends JPanel implements ActionListener, MouseListener, KeyListener {
    ...
    private LinkPicMap linkPicMapUtil;//地图工具类
    ...
    public GamePanel(int count) {
        setSize(600, 600);
        n = 10;
        linkPicMapUtil = new LinkPicMap(count, n);
        map = linkPicMapUtil.getMap();//获取初始时，图片种类为count,行列数为n的地图信息
    ...
    private void getPics() {
        pics = new Image[10];
        for (int i = 0; i <= 9; i++) {
            pics[i] = Toolkit.getDefaultToolkit().getImage("LinkGame/pic" + (i + 1) + ".png");
        }
    }
...
//		boolean isFound = false;
    ...
    public void startNewGame() {
        ...
        count = 0;
        linkPicMapUtil = new LinkPicMap(10, n);
        map = linkPicMapUtil.getMap();
        isClick = false;
        clickId = -1;
        clickX = -1;
        clickY = -1;
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (e.getKeyCode() == KeyEvent.VK_A) {//打乱地图
            map = linkPicMapUtil.getResetMap();
            repaint();
        }
        ...
        if (isClick) {//第二次点击
            ...
            if (map[i][j] != BLANK_STATE) {
                if (map[i][j] == clickId) {//点击的是相同图片Id,但不是重复点击同一图片
                    if (i == clickX && j == clickY)//点击同一位置，直接返回
                        return;
