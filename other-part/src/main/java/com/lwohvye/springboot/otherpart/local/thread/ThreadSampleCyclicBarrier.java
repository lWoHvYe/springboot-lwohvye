...
// CountDownLatch适用于主线程需等子线程结束后再做一些事情，而CyclicBarrier适用于多个线程在做了某些事之后，需要一起做另一件事（等待期间不会释放线程，使用线程池时需注意）
//@SpringBootTest
public class ThreadSampleCyclicBarrier {

    private Logger logger4j = LoggerFactory.getLogger(ThreadSampleCyclicBarrier.class);
...
//            记录开始时间
            ...
            ThreadSampleCyclicBarrier threadSampleCountDownLatch = new ThreadSampleCyclicBarrier();
//            开始模拟
...
//            输出模拟用时
        ...
        } catch (InterruptedException | NoSuchAlgorithmException | BrokenBarrierException e) {
            e.printStackTrace();
        }
    }
